// Modal que muestra los detalles de una orden en Cotizaciones->OrdersAdmin
...
const OrderDetailModal = ({ order, onClose }) => {
    ...
    const calculateTotalAmount = () => {
        return products.reduce((total, product) => total + calculateTotal(product.price, product.quantity), 0);
    };
    ...
    const fetchDiscountedAmount = async () => {
        try {
            ...
            if (response.data && response.data.monto_total) {
                setDiscountedAmount(response.data.monto_total);
            } else {
        ...
        <div className="modal-overlay-details">
                    ...
                    <tbody>
                        {products.map((product, index) => (
                            <tr key={index}>
                                ...
                                <td>{product.price}</td>
                                <td>{calculateTotal(product.price, product.quantity)}</td>
            ...
            <>
                ...
                <div className="total-amount-details">
                    ...
                    <h3>Monto con Descuento: ${discountedAmount}</h3>
                    )} 
                </div>
